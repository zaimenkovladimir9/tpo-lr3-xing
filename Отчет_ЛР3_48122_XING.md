...
`selenium-ide/xing_variant_48122.side`. Все команды поиска элементов
используют `xpath=...`.

Исполняемый набор из 40 автотестов находится в
`src/test/java/ru/tpo/lr3/XingPublicTest.java`. Тесты написаны на Java +
JUnit 5 + Selenium WebDriver и запускаются через Gradle. Selenium RC
снят с поддержки, поэтому фактический запуск выполняется через
WebDriver, но структура сценариев соответствует Selenium IDE-шаблону.

Команда запуска Chrome:
...
Автоматизация выполнена Selenium-тестами с XPath-локаторами. Такой
подход устойчивее к динамически генерируемым `id`, потому что проверки
опираются на пользовательский текст, роли формы и стабильные атрибуты.
